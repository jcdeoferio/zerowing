...
public class OLDClientLauncher extends JFrame implements ClientInterface {
	...
	public void startClient() throws SQLException {
		...
		updateInput
				.setToolTipText("Format of query must satisfy this format:\n SELECT * FROM [YOUR QUERY]");

		changeUnitControlPanel = new JPanel();
		changeUnitControlPanel.setLayout(new BoxLayout(changeUnitControlPanel,
				BoxLayout.Y_AXIS));
		...
		setChangeUnit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				callSetChangeUnit(column.getText(), table.getText(), changeUnit
						.getText());
			}
		});
		...
		updateButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					e.printStackTrace();
					updatesList.setText("SQL Exception: "
		...
		c = Client.getClient(name.getText(), ip.getText(), Integer
				.parseInt(port.getText()), (String) dbType.getSelectedItem(),
				dbIP.getText(), Integer.parseInt(dbPort.getText()), dbName
						.getText(), dbUser.getText(), new String(dbPassword
						.getPassword()));
		if (c == null) {
			System.out
	...
	private void callSetChangeUnit(String column, String table, String cuid) {
		JOptionPane.showMessageDialog(this, "Called Set Change Unit: of col ["
				+ column + "] tab [" + table + "] id: " + cuid);
		if (column == null || table == null || cuid == null)
			return;
		if (column.equals("") || table.equals("") || cuid.equals(""))
			return;
		try {
	...
	public void refreshPeerList() {
		...
		if (peerList.getItemCount() == 0)
			peerList.addItem(new AddressPort(c.getName(), "localhost", c
					.getClientPeerServerPort()));
	}
	...
	public static void main(String args[]) {
		...
		ClientInterface m = new OLDClientLauncher();
	}
	...
	private void loadFromSettings() {
		String[] fieldData = SystemSettings
				.getSystemSettingsArray(OLDClientLauncher.previousLaunch);
	...
	private void saveSettings() {
		...
		SystemSettings.insertSettingsAsArray(toSave,
				OLDClientLauncher.previousLaunch);
	}
